...
========================
IMPORTANT: Drop counting ("enableDropCount": true) does NOT work for Normal Drops!

Drop counting is only available for Event Drops because:
- Normal drops are always active and would create massive data files
- Competitive events need controlled, time-limited tracking
- Event-based counting provides clear start/end boundaries

For drop counting features, use Event Drops instead:
- Create an event folder in Event Drops/
- Add your drop configurations with "enableDropCount": true
- Enable the event when you want tracking to begin
- Use /lootdrops dropcount true to enable the system
